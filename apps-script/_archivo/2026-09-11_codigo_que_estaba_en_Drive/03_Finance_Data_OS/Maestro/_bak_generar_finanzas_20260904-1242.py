...
3. COGS = FEL + tarjeta en las 4 categorias de mercaderia, MAS ALIMENTOS_EFECTIVO
   del banco. NO se suma la categoria ALIMENTOS del banco: ese es el pago de la
   misma factura que ya viene por FEL y se contaria dos veces.
...
5. Las propinas quedan fuera del costo laboral: se cobran al cliente y se
   entregan al equipo. No son costo del negocio.
6. Semana = semana ISO.
...
Uso:  python3 generar_finanzas.py [ruta_al_espejo]
"""
import openpyxl, datetime, json, os, sys, re
from collections import defaultdict

BASE = os.path.dirname(os.path.abspath(__file__))
ESPEJO = sys.argv[1] if len(sys.argv) > 1 else os.path.join(BASE, "Rosanta_Maestro_ESPEJO.xlsx")
OUT = os.path.join(BASE, "_datos_finanzas")
...
MAP = {
 ...
 'PROPINAS_AL_EQUIPO':('Nomina y salarios','V'), 'PROPINAS_PASSTHROUGH':('Nomina y salarios','V'),
    ...
    for hoja, mc, cc, pc in LIBROS:
        ...
        for r in range(5, ws.max_row + 1):
            ...
            if c in FUERA or not c or str(c).startswith('INGRESO'):
                continue
            if c in COGS_CATS:
                if hoja != '03_Banco_Industrial':   # regla 3
                    cog += q
                continue
            if c == 'ALIMENTOS_EFECTIVO':
                cog += q; continue
...
for s in serie:
    ...
    for hoja, mc, cc, pc in LIBROS:
        ...
        for r in range(5, ws.max_row + 1):
            ...
            c = ws.cell(r, cc).value
            q = monto(ws, r, mc, hoja)
            if c in COGS_CATS and hoja != '03_Banco_Industrial':
                cog += q
            elif c == 'ALIMENTOS_EFECTIVO':
                cog += q
            elif c == 'IGSS':
                igss += q
    dev = PLANILLA.get(m)
...
for hoja, mc, cc, pc in LIBROS:
    ...
    for r in range(5, ws.max_row + 1):
        ...
        if w not in sem:
            continue
        if (c in COGS_CATS and hoja != '03_Banco_Industrial') or c == 'ALIMENTOS_EFECTIVO':
            sem[w]['cogs'] += monto(ws, r, mc, hoja)
...
meta_cogs = round(mix_c / 100 * 30 + mix_b / 100 * 20, 1)
...
efe_c = sum(wb['03_Banco_Industrial'].cell(r, 4).value or 0
            ...
            and wb['03_Banco_Industrial'].cell(r, 7).value == 'ALIMENTOS_EFECTIVO')
...
def _destino(c, hoja):
    ...
    if c in COGS_CATS:
        return 'COGS' if hoja != '03_Banco_Industrial' else 'REGLA 3: no suma'
    if c == 'ALIMENTOS_EFECTIVO':                    return 'COGS'
    if c in ('ALQUILERES', 'ALQUILER') and hoja == '01_FEL_Maestro':
...
print("  Regla 3, pago de factura que ya vino por FEL: Q%s" % format(rev, ',.2f'))
