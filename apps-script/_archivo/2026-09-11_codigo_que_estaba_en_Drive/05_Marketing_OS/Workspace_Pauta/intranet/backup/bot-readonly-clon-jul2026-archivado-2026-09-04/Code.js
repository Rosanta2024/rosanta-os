...
function handleInstagram(body) {
  (body.entry || []).forEach(function (entry) {
    (entry.messaging || []).forEach(function (event) {
      ...
      const result = askClaude('instagram', (nombreIG || '').split(' ')[0] || '', text, senderId);
...
function handleMessenger(body) {
  (body.entry || []).forEach(function (entry) {
    (entry.messaging || []).forEach(function (event) {
      ...
      const result = askClaude('messenger', '', text, senderId);
...
'    "email": "<correo del cliente si lo dio, si no vacío>",',
  '    "fecha": "<fecha del evento si la dio, si no vacío>",',
