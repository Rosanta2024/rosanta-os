 ...
 *
 *  VALIDACION DE SALIDA (31-ago-2026): antes de publicar se limpia y se valida
 *  el texto que devuelve el modelo. Si trae razonamiento interno, varios
 *  borradores o no parece una respuesta, NO se publica: cae a aprobacion
 *  manual aunque sean 4 o 5 estrellas.
 *  Motivo: el 28-jul (Lukas Lackner) y el 28-ago (Sylvain Adnet) se publicaron
 *  en Google dos respuestas con la deliberacion del modelo pegada al texto
 *  bueno. Nada las detuvo porque se publicaba content[0].text tal cual.
 * ============================================================
 */
...
function procesarResenas() {
  ...
  reviews.forEach(function (r) {
    ...
    const red = redactarRespuesta_(estrellas, nombre, texto);

    if (estrellas >= CONFIG.AUTO_MIN && red.confiable) {
      // ---- Auto-publicar (4-5 estrellas y salida limpia) ----
      publicarRespuesta_(reviewId, red.texto);
      registrar_(sheet, r, red.texto, 'PUBLICADA_AUTO', '');
      auto++;
    } else {
      // ---- Dejar para aprobación ----
      // Caen aquí las de 1-3 estrellas y también las de 4-5 cuyo texto no pasó
      // la validación. Preferimos que Juanma revise a publicar a ciegas.
      registrar_(sheet, r, red.texto, 'PENDIENTE_APROBAR', 'NO');
      paraAprobar++;
    }
  });
...
function redactarRespuesta_(estrellas, nombre, textoResena) {
  ...
  if (res.getResponseCode() !== 200) {
    ...
    // El fallback está escrito a mano, así que sí es publicable.
    return { texto: respuestaFallback_(estrellas, nombre), confiable: true };
  }

  const json  = JSON.parse(res.getContentText());
  const crudo = (json.content && json.content[0] && json.content[0].text || '').trim();
  const limpio = sanearRespuesta_(crudo, nombre);

  if (respuestaPublicable_(limpio)) return { texto: limpio, confiable: true };

  // No se descarta el texto: se manda a aprobación con lo que se pudo salvar,
  // para que Juanma lo edite en vez de perder la respuesta.
  Logger.log('Salida NO publicable, va a aprobación. Crudo: ' + crudo.slice(0, 500));
  return { texto: limpio || respuestaFallback_(estrellas, nombre), confiable: false };
}

// ===================== VALIDACIÓN DE LA SALIDA DEL MODELO =====================
// Dos fallos reales que esto ataca:
//   28-jul-2026, Lukas Lackner (5★): respuesta correcta en alemán + un bloque
//     "**[Note visible only to you]:**" preguntando si el alemán era aceptable.
//   28-ago-2026, Sylvain Adnet (4★): la deliberación completa sobre la regla de
//     "nuestro" y TRES borradores alternativos, uno detrás de otro.
// En los dos casos el texto bueno estaba al principio. Por eso primero se
// intenta salvar (cortar en el primer marcador) y solo después se descarta.

const RESP_MIN_CHARS = 40;
const RESP_MAX_CHARS = 600;

// Marcadores que solo aparecen cuando el modelo se puso a pensar en voz alta.
// Ante la duda se prefiere un falso positivo: el costo es que Juanma revise
// una respuesta buena, no que Google publique una mala.
const MARCADORES_META = [
  '[Note', '[note', '[NOTE', '[Nota', '[nota',
  'visible only to you', 'visible solo para',
  'Wait,', 'Wait ', 'Espera,',
  'Let me ', 'Let\'s ', 'Veamos si', 'Revisemos',
  'I should ', 'I need to ', 'I used ', 'I responded ', 'I must ', 'I also ',
  'Actually, I', 'Actually, it', 'En realidad, ',
  'my rules', 'your rules', 'the rules', 'las reglas', 'la regla ',
  'the review was', 'The review was',
  '```', '⸻', '\n---', '\n***'
];

// Corta el meta-texto y se queda con el primer borrador.
function sanearRespuesta_(texto, nombre) {
  let t = String(texto || '').replace(/\r/g, '').trim();
  if (!t) return '';

  // 1) Cortar en el primer marcador de razonamiento interno.
  let corte = t.length;
  MARCADORES_META.forEach(function (m) {
    const i = t.indexOf(m);
    if (i > -1 && i < corte) corte = i;
  });
  t = t.slice(0, corte).trim();

  // 2) Si el saludo se repite, el modelo escribió varios borradores seguidos.
  //    Nos quedamos con el primero.
  const primerNombre = String(nombre || '').trim().split(/\s+/)[0];
  if (primerNombre.length > 2) {
    const segundo = t.indexOf(primerNombre + ',', 1);
    if (segundo > -1) t = t.slice(0, segundo).trim();
  }

  // 3) Restos de formato: negritas, comillas envolventes y separadores sueltos.
  t = t.replace(/\*\*/g, '')
       .replace(/^["“«']+/, '')
       .replace(/["”»']+$/, '')
       .replace(/[\s*_=–—-]{3,}$/, '')
...
// ¿Esto se puede publicar en la ficha pública sin que nadie lo mire antes?
function respuestaPublicable_(texto) {
  const t = String(texto || '').trim();
  if (!t) return false;
  if (t.length < RESP_MIN_CHARS || t.length > RESP_MAX_CHARS) return false;

  // Si sigue quedando algún marcador dentro, no se publica.
  for (let i = 0; i < MARCADORES_META.length; i++) {
    if (t.indexOf(MARCADORES_META[i]) > -1) return false;
  }

  // Nunca debe salir la instrucción del prompt ni un encabezado de rol.
  if (/^(respuesta|response|draft|borrador|opción|option)\s*\d*\s*:/i.test(t)) return false;

  // Regla del prompt: máximo 3 oraciones. Más de 5 es señal de varios borradores.
  const oraciones = (t.match(/[.!?…]+(\s|$)/g) || []).length;
  if (oraciones > 5) return false;

  // Debe terminar en punto final: si no, es que la cortamos a media frase.
  // OJO: no meter ! ni ? en esta clase, son cierre válido de oración.
  const sinAdornos = t.replace(/[\s"'»)\]\u2190-\u2BFF\uD83C-\uDBFF\uDC00-\uDFFF\uFE0F]*$/, '');
  const ultimo = sinAdornos.slice(-1);
  if ('.!?…'.indexOf(ultimo) === -1) return false;
...
'- Si la reseña viene en otro idioma (alemán, italiano, francés, hebreo...), responde en inglés. No preguntes ni comentes nada sobre el idioma: solo responde.',
...
'No escribas notas para mí, ni alternativas, ni razonamiento, ni te corrijas a media respuesta. Un solo texto, el definitivo. Lo que devuelvas se publica en Google tal cual.'
  ].join('\n');
}
...
function notificarPendientes_(sheet) {
  ...
  let cuerpo = 'Hola Juanma,\n\nHay ' + pendientes.length +
    ' reseña(s) esperando tu aprobación antes de publicar.\n' +
    '(Son las de 1 a 3 estrellas, más cualquiera de 4-5 cuyo borrador no pasó la validación.)\n\n';
