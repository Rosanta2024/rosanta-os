...
function crearHojaCosteo() {
  ...
  idx.getRange(1, 1, 1, 7).setValues([[
    'PRODUCTO', 'PROVEEDOR', 'RECETA', 'TIPO', 'CANTIDAD', 'UNIDAD', 'COSTO_LINEA'
  ]]).setFontWeight('bold');
  idx.setFrozenRows(1);
...
function getHistorialPrecios(producto) {
  ...
  return hoja.getRange(2, 1, hoja.getLastRow() - 1, 9).getValues()
    ...
    .map(function (f) {
      return {
        fecha: f[0] instanceof Date ? f[0].toISOString().slice(0, 10) : String(f[0]),
 ...
 */
function registrarPrecio(datos) {
  var usuario = getUsuarioActual();
  if (!usuarioTieneModulo(usuario, 'recetario')) throw new Error('Sin acceso al recetario');
  ...
  if (factor === null) throw new Error('No se puede convertir: el Banco de Datos no tiene precio de compra para "' + ubic.producto + '". Corregirlo a mano primero.');
  ...
  hojaCosteo_().getSheetByName(COSTEO.hojas.precios).appendRow([
    ...
    nuevoUnidad, datos.factura || '', (usuario && usuario.email) || Session.getActiveUser().getEmail(),
    datos.nota || ''
  ]);
  ...
  return {
    ...
    recetasAfectadas: ins ? ins.usos.map(function (rid) {
      ...
      return { nombre: r.nombre, tipo: r.tipo, cmv: r.cmv, estado: r.estado };
    }) : []
  };
}

/** Ubica un producto dentro de la hoja BANCO DE DATOS (busca en todas las areas). */
function ubicarEnBanco_(producto) {
...
/** Vuelca el indice insumo -> receta a una hoja, para poder consultarlo tambien desde Sheets. */
function regenerarIndice() {
  ...
  modelo.recetas.forEach(function (r) {
    r.ingredientes.forEach(function (g) {
      ...
      filas.push([g.nombre, i ? i.proveedor : 'FUERA DEL BANCO', r.nombre, r.tipo,
 ...
 * Correr desde el editor y leer el Log, o llamarlo desde la vista.
 */
function reporteHigiene() {
  var m = construirModelo_();
  var fuera = [], sinUso = [], sinPrecioCompra = [], vacias = [], sobreMeta = [];

  m.recetas.forEach(function (r) {
    if (!r.ingredientes.length) vacias.push(r.nombre);
    else if (r.estado === 'alto') sobreMeta.push(r.nombre + ' (' + r.cmv.toFixed(1) + '%)');
    r.ingredientes.forEach(function (g) {
      if (g.insumo === null) fuera.push(r.nombre + ' > ' + g.nombre);
    });
  });
