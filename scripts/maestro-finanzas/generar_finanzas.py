...
8. RETIRADA el 23-sep-2026 (Juanma). Saltaba el pago de una lista de proveedores
   asumiendo que su factura entraba por FEL, sin verificarlo: lo que cobraban de
   mas desaparecia del DRE. Medido el 22-sep sobre el año: Edwin Flores pagaba
   Q18,500 y facturaba Q14,000, EX Security Q3,000 contra Q2,250 — Q5,250 de gasto
   real sin aparecer. La cubre la regla 15, que casa el pago con su factura ANTES
   de saltarlo. En la intranet era la regla 9 (alla el 8 ya estaba tomado) y se
   retiro el mismo dia: las dos tienen que dar el mismo DRE.
15. LA FACTURA MANDA (Juanma, 21-sep-2026): se le da prioridad a la factura y el
   banco solo cuenta si no existe factura. Un pago de banco o tarjeta que casa con
   una factura FEL (mismo bloque del DRE, mismo monto, factura entre 45 dias antes
   y 10 despues del pago) no se suma. Generaliza las reglas 7 y 8, que dependian
   de listas. Medido al entrar: 54 pagos, Q13,950 del año. Ver casar_pagos().
...
FUERA = {'DEVOLUCION_INVERSION','CARGO_FRAUDULENTO','PAGO_TARJETA_CREDITO','PAGO_TARJETA',
         ...
         'ANULADA',   # regla 13
         'FACTURA_AJENA'}   # 21-sep-2026: ni del restaurante ni personal
...
# ---------- regla 15 (21-sep-2026, Juanma): LA FACTURA MANDA ----------
# "Se le da prioridad a las facturas; si no existe factura, se usan los bancos."
# Un pago de banco o tarjeta que tiene su factura FEL no suma: es el pago de esa
# factura. Las reglas 7 y 8 lo resolvian por categoria y por lista de proveedores,
# y cada proveedor nuevo volvia a contarse dos veces hasta que alguien lo veia
# (medido el 21-sep: 54 pagos, Q13,950 del año, las dos abogadas incluidas).
# Ahora se casa cada pago con UNA factura: mismo bloque del DRE, mismo monto y
# factura emitida entre 45 dias antes y 10 dias despues del pago. Una factura casa
# con un solo pago. Mismo algoritmo en FinanzasDatos.gs (_finCasarPagos_).
# Solo en los bloques donde el gasto viene con factura de proveedor. Nomina,
# propinas, impuestos, comisiones e inmueble quedan fuera a proposito.
FACTURA_MANDA_BLOQUES = {'Tarifas y servicios', 'Prestadores y honorarios', 'Marketing',
                         'Mantencion', 'Bienes de uso', 'Uniformes'}
FM_DIAS_ANTES, FM_DIAS_DESPUES = 45, 10   # la factura, respecto del pago


def casar_pagos(facturas, pagos):
    """facturas y pagos: listas de dict con d (date), q, bloque y llave. Devuelve
    {llave del pago: llave de la factura}. Los pagos se recorren por fecha; gana la
    factura mas cercana en dias y, a igual distancia, la primera de la hoja."""
    usadas, casados = set(), {}
    for p in sorted(pagos, key=lambda x: (x['d'], x['orden'])):
        mejor, dist = None, None
        for f in facturas:
            if f['llave'] in usadas or f['bloque'] != p['bloque']:
                continue
            if abs(f['q'] - p['q']) > 0.011:
                continue
            dd = (p['d'] - f['d']).days
            if dd < -FM_DIAS_DESPUES or dd > FM_DIAS_ANTES:
                continue
            if dist is None or abs(dd) < dist:
                mejor, dist = f, abs(dd)
        if mejor:
            usadas.add(mejor['llave'])
            casados[p['llave']] = mejor['llave']
    return casados


def _pagos_con_factura():
    facturas, pagos = [], []
    for i, (hoja, mc, cc, pc) in enumerate(LIBROS):
        ws = wb[hoja]
        for r in range(5, ws.max_row + 1):
            d = dia(ws.cell(r, 1).value)
            if not (isinstance(d, datetime.datetime) and d.year == ANIO):
                continue
            c = ws.cell(r, cc).value
            if ws.cell(r, pc).value == 'Sí' or c not in MAP or MAP[c][0] not in FACTURA_MANDA_BLOQUES:
                continue
            q = round(float(monto(ws, r, mc, hoja) or 0), 2)
            if q <= 0:
                continue
            fila = {'d': d.date(), 'q': q, 'bloque': MAP[c][0], 'llave': (hoja, r), 'orden': (i, r)}
            if hoja == '01_FEL_Maestro':
                facturas.append(fila)
            # Los pagos de la regla 8 quedaban fuera de este emparejamiento: se los
            # saltaba antes. Retirada la regla, entran y la 15 decide con la factura.
            elif c not in SOLO_FEL:
                pagos.append(fila)
    return casar_pagos(facturas, pagos)


PAGO_CON_FACTURA = _pagos_con_factura()
...
for m in range(1, 13):
    ...
    for hoja, mc, cc, pc in LIBROS:
        ...
        for r in range(5, ws.max_row + 1):
            ...
            if (hoja, r) in PAGO_CON_FACTURA:
                continue                             # regla 15: la factura manda
            if c in MAP:
                bl[MAP[c][0]] += q
                tip[MAP[c][1]] += q
...
for hoja, mc, cc, pc in TODOS_LOS_LIBROS:
    ...
    for r in range(5, ws.max_row + 1):
        ...
        dest = ('anulada en SAT' if c == 'ANULADA'
                else 'personal' if ws.cell(r, pc).value == 'Sí' else _destino(c, hoja))
        if dest.startswith('DRE') and (hoja, r) in PAGO_CON_FACTURA:
            dest = 'REGLA 15: tiene factura FEL'
...
# El total por NIT del FEL se leia aca para el reporte de la regla 8, que vigilaba
# que cada proveedor de la lista siguiera facturando. Retirada la regla el
# 23-sep-2026, ya nadie lo usaba: se va con ella.
...
print("  Reglas 3 y 7, y pagos casados con su factura FEL (regla 15): Q%s" % format(rev, ',.2f'))
print("  Cada uno de esos pagos tiene su factura: la regla 15 la encontro antes de saltarlo.")
